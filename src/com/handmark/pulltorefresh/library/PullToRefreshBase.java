...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
	...
	public interface OnRefreshTouchListener {
		public void onRefreshTouchMove();

		public void onRefreshTouchUp();
	}

	OnRefreshTouchListener onRefreshTouchListener;
	...
	public final boolean onTouchEvent(MotionEvent event) {
		...
		if (onRefreshTouchListener != null) {
			onRefreshTouchListener.onRefreshTouchMove();
		}
		...
		case MotionEvent.ACTION_UP: {
			if (onRefreshTouchListener != null) {
				onRefreshTouchListener.onRefreshTouchUp();
			}
			if (mIsBeingDragged) {
				mIsBeingDragged = false;
	...
	public void setLastUpdatedLabel(CharSequence label) {
		String timeLabel = new SimpleDateFormat("yyyy-MM-dd hh:mm:ss").format(new java.util.Date());
		getLoadingLayoutProxy().setLastUpdatedLabel(timeLabel);
	}
	...
	protected void onPullToRefresh() {
		switch (mCurrentMode) {
		case PULL_FROM_END:
			mFooterLayout.initPullToRefresh();
			mFooterLayout.pullToRefresh();
			break;
		case PULL_FROM_START:
			mHeaderLayout.initPullToRefresh();
	...
	protected final void setHeaderScroll(int value) {
		...
		case VERTICAL:
			scrollTo(0, value);
			float progress = (float) (Math.abs(value)) / getHeaderSize();
			if (progress >= 1) {
				progress = 1;
			}
			mHeaderLayout.setProgress(progress);
			break;
		case HORIZONTAL:
			scrollTo(value, 0);
			break;
	...
	private void pullEvent() {
		...
		if (scrollYListener != null) {
			scrollYListener.scrolly(newScrollValue);
		}
	...
	public ScrollY scrollYListener;

	public void setScrollYListener(ScrollY scrollYListener) {
